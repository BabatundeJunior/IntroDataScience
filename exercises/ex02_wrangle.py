...
def _(pl, students):
    ...
    print(grade_10_good_attendance)
...
def _(pl, students):
    ...
    subset = students.select(pl.col("name"), pl.col("grade_level"), pl.col("test_score"))  # Use students.select(...)
    # print(subset.head)
    subset.head()
    return
...
def _(pl, sales):
    ...
    date_range = sales.select( pl.col("date").min().alias("start_date"), pl.col("date").max().alias("end_date"))
    print(date_range)
    date_range.head()
    return
...
def _(pl, sales):
    ...
    sales_with_month = sales.with_columns(
        ...
        pl.col("date").str.to_date().dt.month().alias("month"))  # Use with_columns() and pl.col().str.to_date()
...
def _(pl, sales_with_month):
    ...
    monthly_sales = sales_with_month.group_by("month").agg(pl.col("total_amount").sum().alias("total_sales")).sort("total_sales", descending=True)
